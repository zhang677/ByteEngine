...
std::tuple<Tensor, Tensor> TransformerEncoder(int64_t head_num, int64_t head_size, int64_t num_layers, 
  ...
  // [hk:] pass num_layers for chatGLM
  btencoder->forward(batch_size, seq_len, num_layers, input, attr_mask, output, qkv_cache, is_remove_padding,
                     use_fused_attention);
  delete btencoder;
...
static auto registry = torch::RegisterOperators(
    ...
    "int head_num, int head_size, int num_layers,"
    "Tensor qkv_kernel, Tensor qkv_bias,"
    "Tensor attr_output_kernel, Tensor attr_output_bias,"
    "Tensor attr_output_layernorm_gamma, Tensor attr_output_layernorm_beta,"
