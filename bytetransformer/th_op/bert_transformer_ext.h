...
class IBTEncoder {
 public:
  ...
  virtual void forward(int batch_size, int seq_len, int num_layers, Tensor &input, Tensor &attr_mask,
...
class BTEncoder : public IBTEncoder {
 public:
  ...
  // [hk:] one parameter 'num_layers' should be passed into the block
  void forward(int batch_size, int seq_len, int num_layers, Tensor &input, Tensor &attr_mask, Tensor &output, Tensor &qkv_cache, 
    ...
    struct BertTransformerInferParam<T> infer_param {
      from_tensor, atten_mask, transformer_out, qkv_cache_ptr, buf, batch_size, seq_len, num_layers, cublas_handle, stream
    };
