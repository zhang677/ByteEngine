...
class BertTransformer : public torch::jit::CustomClassHolder {
 public:
  ...
  Tensor forward(Tensor input, Tensor attr_mask, int64_t num_layers) {
    return forwardv2(input, attr_mask, num_layers, true, true);
  }

  Tensor forwardv2(Tensor input, Tensor attr_mask, int64_t num_layers, bool is_remove_padding,
    ...
    btencoder->forward(batch_size, seq_len, num_layers, input, attr_mask, output, buf, is_remove_padding,
                       use_fused_attention);
    return output;
  }
