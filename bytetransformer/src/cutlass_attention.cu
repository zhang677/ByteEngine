...
void CutlassAttention<OpType>::infer_impl(AttentionInferParam *infer_param) {
  ...
  // switch on is_roformer for chatGLM
  // const bool is_roformer = true;
  // add bias
  if constexpr (true) {
    dim3 grid, block;
    ...
    if (this->is_remove_padding_) {
      ...
      if (block.x <= 1024){
        ...
        // [hk:] this kernel add query /= sqrt(hidden_size)
        add_QKV_bias_large_dim<<<grid, dim3(512), 0, stream>>>(
          infer_param->qkv, this->param_.attr_bias_QKV, query, key, value, batch_size,
          seq_len, head_num, size_per_head_half, is_roformer);
      }
    }
  }
