...
__global__
  ...
  for (int seq_id = quart_warpId; seq_id < seq_len; seq_id += warpNums * 4) {
    ...
    *(__half2 *)(*s_kv + offset) = __hadd2(__ldg(&qkv[pos + half_hidden_dim]), k_bias);
    ...
    // half2 q_value = __hadd2(__ldg(&qkv[pos]), q_bias);
    // half2 k_value = __hadd2(__ldg(&qkv[pos + half_hidden_dim]), k_bias);
    
    // rotary embedding added for chatGLM
    ...
    // store into shmem
    // *(__half2 *)(*s_query + offset) = q_value;
    // *(__half2 *)(*s_kv + offset) = k_value;
  }
...
#pragma unroll
  ...
  for (int seq_id = quart_warpId; seq_id < seq_len; seq_id += warpNums * 4) {
    ...
    // [hk:] copy out
    qkv[pos + half_hidden_dim * 2] = ((__half2 *)(s_kv[seq_id]))[quart_warp_tid];
  }
...
#pragma unroll
    ...
    ((__half2 *)(s_kv[from_id]))[warp_tid] =
        __hadd2(__ldg(&qkv[pos + half_hidden_dim * 2]), v_bias);
    // [hk:] copy out
    qkv[pos + half_hidden_dim * 2] = ((__half2 *)(s_kv[from_id]))[warp_tid];
  }
