...
struct BertTransformerInferParam {
  ...
  int batch_size;
  int seq_len;
  // [hk:] add an extra parameter num_layers
  int num_layers;
...
class BertTransformer {
 ...
 public:
  BertTransformer(const int max_batch_size, const int head_num, const int size_per_head,
    ...
    // printf("CUTLASS Attention is defined !"); [True]
    // [hk]: set 'arch_ == 86' to 'arch_ >= 80' to use CUTLASS
    ...
    if (use_cutlass) {
      if constexpr (OpType == OperationType::HALF) {
        ...
        // printf("CUTLASS Attention is used !"); [True] (1 1024 12 64)
      } else {
  ...
  unsigned long long cal_bufsize() {
    ...
    // [hk:] change "5" to "6" for an extra layernorm in chatGLM
